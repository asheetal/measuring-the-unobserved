...
FLAGS <- flags(
  ...
  flag_numeric("R_FLAGS_DROPOUT", 0.2), #May 25 from 0.1
...
my_hiddens <- function (in_object = NULL) {
...
if (TYPE == 1) {
  output <- input.constructs %>% 
    my_hiddens()
} else if (TYPE == 2) {
  output <- layer_concatenate(list(model.asian, model.black, model.hispanic, model.white)) %>% 
    my_hiddens()
} else {
...
model_file <- filename(paste0("model_output_", TYPE),
                       path=basedir,
                       tag=NULL,
                       ext="hdf5",
                       subdir=FALSE) %>%
...
options(filenamer.timestamp=1)
training_image <- filename(paste0("training_", TYPE),
                           path=basedir,
                           tag=NULL,
                           ext="png",
                           subdir=FALSE) %>%
