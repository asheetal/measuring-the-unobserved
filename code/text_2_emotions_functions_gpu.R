...
my_decipher_ethnicity <- function(prof_names = NULL) {
  tdf <- data.frame(text = prof_names)
  ethnicity_probs <- add_ethnicity(tdf)
  
  ethnicity_probs_grouped <- ethnicity_probs %>%
  ...
  ethnicity_coded <- colnames(ethnicity_probs_grouped)[max.col(ethnicity_probs_grouped)]
  return(ethnicity_coded)
}
